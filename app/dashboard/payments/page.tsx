                ...
                <>
                    ...
                    <div className="md:hidden space-y-4">
                        {bookings.map((booking) => {
                            ...
                            return (
                                <div key={booking.id_pemesanan} className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
                                    <div className="flex justify-between items-start mb-3">
                                        <div>
                                            <p className="text-xs font-mono text-gray-500">#{booking.id_pemesanan}</p>
                                    ...
                                    <div className="flex justify-between items-center text-sm border-t border-gray-100 pt-3 mt-2">
                                        <div className="flex flex-col">
                                            ...
                                            <span className="font-bold text-[#31694E]">Rp {displayAmount.toLocaleString('id-ID')}</span>
                                        </div>
                                        <div className="text-right">
                                            <p className="text-xs text-gray-400 mb-1">{format(booking.tgl_masuk, 'dd MMM', { locale: id })} - {format(booking.tgl_keluar, 'dd MMM', { locale: id })}</p>
                                            {(booking.status === 'Menunggu Pembayaran' || booking.status === 'Menunggu') && (
                                                <Link
                                                    href={`/dashboard/payment/${booking.id_pemesanan}`}
                                                    className="inline-block px-4 py-1.5 bg-[#658C58] text-white text-xs font-medium rounded-lg hover:bg-[#31694E] transition-colors"
